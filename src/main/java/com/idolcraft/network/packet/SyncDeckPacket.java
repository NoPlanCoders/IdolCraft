    ...
    public static void handle(SyncDeckPacket msg, Supplier<NetworkEvent.Context> ctx) {
        ctx.get().enqueueWork(() ->
                net.minecraftforge.fml.DistExecutor.unsafeRunWhenOn(Dist.CLIENT, () -> () ->
                        com.idolcraft.client.ClientDeckState.update(msg.hand, msg.handUsable, msg.selectedIndex, msg.focusStacks, msg.goodTicks, msg.greatTicks, msg.pLevel, msg.produceXp)
                )
        );
