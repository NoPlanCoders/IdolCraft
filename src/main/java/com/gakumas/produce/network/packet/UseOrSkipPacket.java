...
public class UseOrSkipPacket {
    ...
    public static void handle(UseOrSkipPacket msg, Supplier<NetworkEvent.Context> ctx) {
        ctx.get().enqueueWork(() -> {
            ...
            // 手帳を所持していない場合は無効
            boolean holdingHandbook = player.getMainHandItem().getItem() instanceof com.gakumas.produce.item.HandbookItem
                    || player.getOffhandItem().getItem() instanceof com.gakumas.produce.item.HandbookItem;
